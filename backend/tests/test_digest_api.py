...
@pytest.mark.asyncio
async def test_generate_template_continues_when_context_fetch_fails(client: AsyncClient):
    async with async_session() as session:
        topic = Topic(title="Resilient Draft Topic", date=date(2026, 3, 1), position=0)
        session.add(topic)
        await session.commit()
        await session.refresh(topic)

        tweet = Tweet(
            tweet_id="template_fetch_fail",
            author_handle="testuser",
            author_display_name="Test User",
            text="Template tweet text",
            url="https://x.com/testuser/status/template_fetch_fail",
            saved_at=datetime(2026, 3, 1, 12, 0, tzinfo=timezone.utc),
        )
        session.add(tweet)
        await session.commit()
    ...
    with (
        ...
        mock_grok.side_effect = RuntimeError("context fetch timed out")
